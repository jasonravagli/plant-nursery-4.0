...
public class PositionController extends BaseController {
	...
	public boolean updatePosition(PositionDto positionDto) {
		...
		GrowthPlace growthPlace = growthPlaceDao.findById(positionDto.getGrowthPlaceId());
		...
		Position position = positionDao.findById(positionDto.getId());
		...
		if(position.getGrowthPlace().getId() != positionDto.getId()) {
			this.setErrorOccurred(true);
			this.setErrorMessage("Cannot move positions between growth places");
			return false;
		}
		
		if(position.getRowIndex() != positionDto.getRowIndex() || position.getColumnIndex() != positionDto.getColIndex()) {
			this.setErrorOccurred(true);
